...
const LandingPage: React.FC<LandingPageProps> = ({ user }) => {
  ...
  const menuItems = [
    ...
    { name: "Messages", path: "/messages", icon: RiMessage2Line },
    { name: "Logout", path: "/", icon: RiLogoutBoxRLine },
  ];
                ...
                {isDropdownOpen && (
                  <div className=" mt-8 bg-white shadow-lg">
                    {menuItems.map((item, index) => (
                      ...
                      >
                        <item.icon className="mr-2 text-black-400" />
